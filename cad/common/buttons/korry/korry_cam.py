...
def cam_sleeve(name, width, height, stock_thickness, corner_radius=3.175/2, left_ledge=True, right_ledge=True, show_object=None):
    ...
    output_name = [
        name,
        f"{width}x{height}x{stock_thickness}",
        f"r{corner_radius}"
    ]
    if not left_ledge:
...
def cam_slider(name, width, height, stock_thickness, corner_radius=3.175/2, show_object=None):
...
cam_sleeve("korry/sleeve", 19.5, 19.5, 4, left_ledge=True, right_ledge=True, show_object=show_object)
cam_sleeve("korry/sleeve", 19.5, 19.5, 4, left_ledge=False, right_ledge=True)
